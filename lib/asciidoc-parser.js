...
module.exports = class AsciiDocParser {
  ...
  readHeader () {
    ...
    if ((nextLine = this.peekLine()) && !nextLine.isBlank() &&
      ...
      const headerText = nextLine.rawText.replace(/^=+\s+/, '')
  ...
  readSection (level, dataLines) {
    const titleLine = this.readLine(),
      ...
      headerText = titleLine.rawText.replace(/^=+\s+/, ''),
      sectionTitle = {
        type: 'Header',
        depth: level + 1,
        loc: {
  ...
  readDelimitedBlock (dataLines, opts = {}) {
    const startDelimiterLine = this.readLine(),
      ...
      blockType = delimiterInfo[0] === 'listing' ? 'CodeBlock' : 'DelimitedBlockNode',
      block = {
        type: blockType,
  ...
  readParagraph (dataLines) {
    ...
    while ((nextLine = this.peekLine())) {
      if (block) {
        ...
        if (nextLine.rawText.startsWith(" ")) {
          block = {
            type: 'CodeBlock',
